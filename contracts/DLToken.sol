...
contract DLToken{
  ...
  function transfer(address _receiver, uint256 _amountOfToken) external {
    ...
    uint256 amountToBurn = (_amountOfToken * 5) / 100;
    uint256 transferAmount = _amountOfToken - amountToBurn;
    ...
    uint256 transferAmount = _amountOfToken - amountToBurn;
        ...
        balances[_buyer] = balances[_buyer] + transferAmount;

        totalSupply -= amountToBurn;


        emit Transfer(_owner, _buyer, _amountOfToken);
        emit Transfer(_owner, address(0), amountToBurn);
    }
